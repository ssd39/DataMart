...
object ML0NodeContext {
  ...
  object syntax {
    ...
    implicit class dataStateOps[F[_]: Async: SecurityProvider](
      ...
      def insert(batch: SortedSet[Signed[DMUpdate]])(implicit
        ev: SignedUpdateReducer[F, DMUpdate, DataState[OnChain, CalculatedState]]
      ): F[DataState[OnChain, CalculatedState]] =
        ev.foldLeft(dataState, batch)

      def verify(batch: NonEmptyList[Signed[DMUpdate]])(implicit
        ev: LatestUpdateValidator[F, Signed[DMUpdate], DataState[OnChain, CalculatedState]]
