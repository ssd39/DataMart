...
object ML0Service {
  ...
  def make[F[+_]: Async: SecurityProvider: Hasher: JsonSerializer: Logger]: F[BaseDataApplicationL0Service[F]] = for {
    ...
    combiner = SignedUpdateReducer.make[F, DMUpdate, DataState[OnChain, CalculatedState]](StateUpdateCombiner.make[F])
    validator = LatestUpdateValidator.make[F, Signed[DMUpdate], DataState[OnChain, CalculatedState]](
      ML0Validator.make[F]
    )
  ...
  private def makeBaseApplicationL0Service[F[+_]: Async: SecurityProvider: Hasher: JsonSerializer: Logger](
    ...
    combiner:          SignedUpdateReducer[F, DMUpdate, DataState[OnChain, CalculatedState]],
    validator:         LatestUpdateValidator[F, Signed[DMUpdate], DataState[OnChain, CalculatedState]]
  ): BaseDataApplicationL0Service[F] =
    BaseDataApplicationL0Service[F, DMUpdate, OnChain, CalculatedState](
      new DataApplicationL0Service[F, DMUpdate, OnChain, CalculatedState] {
        ...
        override def validateUpdate(update: DMUpdate)(implicit
          context: L0NodeContext[F]
